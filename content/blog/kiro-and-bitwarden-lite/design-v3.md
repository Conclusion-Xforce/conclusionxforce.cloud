...
# SSL (ACM certificate or self-signed)
...
Access Bitwarden at:
  HTTP:  http://bitwarden.example.com:8080
  HTTPS: https://bitwarden.example.com:8443
...
{
  "bitwardenFqdn": "bitwarden.example.com",  // Your domain name
  "hostedZoneName": "example.com",            // Your Route53 hosted zone
...
3. **Implement Lambda Functions** (Python 3.11+)
   - Start container handler (`lambda/start-container/index.py`)
     - Update Security Group (ports 8080 & 8443)
     - Start ECS task
...
- **Dynamic DNS**: Lambda updates A record to point to current container IP on start
- **DNS Cleanup**: Step Functions removes A record on shutdown
- **ACM Certificate**: Optional - use ACM for proper SSL certificate with your domain
